//! Library Checker (<https://judge.yosupo.jp>) problem handler.
...
fn source_directory(cache_dir: &Path, problem_id: &str) -> Result<PathBuf> {
...
fn generate_test_cases(cache_dir: &Path, problem_id: &str) -> Result<()> {
    let repo = repo_path(cache_dir);
...
fn discover_test_cases(source_dir: &Path) -> Result<Vec<TestCase>> {
...
pub(super) fn download_and_generate(
    cache_dir: &Path,
    problem_id: &str,
    url: &str,
    file: &Path,
